...
核心刻度；「今天值得看的」固定放在核心刻度上方。核心刻度是唯一的一张表。
展示层把七档折叠成四个类别指数：超大厂（MSFT/AMZN/GOOGL/META/ORCL）、
受监管公用事业、数据中心 REIT、纯算力商；超大厂只是展示合并，底层七档归属、
档间差距与判断框架不变。类别指数行下面按发行人逐行展开 5–10Y 子读数，
随后保留四个跨档距离、离散度与 SPV 租户差。指数行带
`1D / 1W / 1M / vs 锚点` 四档变化；子项行展示当前读数与样本只数。
**水平值本身信息量很低，驱动判断的是位移**，所以变化量仍是指数行的主体。
