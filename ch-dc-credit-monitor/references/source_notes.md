...
| SPDR 历史持仓 | 端点只有当日快照，日期参数被忽略（下面有实测），无 archive 路径 |
| 新发定价与发行折价 | 一级市场指引→定价、认购倍数无免费结构化源，走人工录入 |

建空列会让面板长期挂着永远是 NULL 的格子，所以这四项**根本不建列**。

## 价格层没有历史，基准层有

这条决定了「回补」这件事能做到哪一步，值得单独记。

**SPDR 只给当日快照，日期参数是装饰。** 2026-08-30 实测：
`holdings-daily-us-en-spbo.xlsx` 与 `holdings-daily-us-en-spbo.xlsx?date=20260630`
拿回来的文件**字节完全相同**（sha1 `b291bf2621a5`，377065 字节）；
`...-spbo-20260630.xlsx` 与 `archive/...?asOfDate=` 两种路径都 404。
逐 CUSIP 的历史价格也没有免费源（见上表的逐笔 TRACE）。结论：
**发行人层的 G-spread 序列只能从开始采集那天往后长**，派生层（`drv.*`）跟着它走。

**FRED 给完整序列，想要多久有多久。** 所以基准层可以一次性补满：
`scripts/backfill.py --days 200` 实测落 138 天国债曲线 + 144 天指数 OAS
（其余是周末与假期）。基准层以前根本没落库——collect 取完在内存里算完利差就扔，
metrics 每次重打一次 FRED。代价是判据 1 的市场 beta 依赖实时网络调用，
锚点日的指数 OAS 也无处可查，而剔市场口径的漂移正需要它。现在
`collect.py` 每天回写最近 30 天（幂等 upsert 顺带吃掉 FRED 的回溯修订），
metrics 先读库再用实时值覆盖重叠日期，FRED 挂掉的当天仍然有基准可用。
