...
0. **首次部署先回填**：`python scripts/backfill.py`。两路历史一次拉满——Ornn 的 90 天
   ...
   之后这两条历史由每日采集自动续上：Ornn 自带 `--history-days 120` 重拉，
   OpenRouter 周度量在采集 openrouter 时全量重取并幂等 upsert（最后一个未结算周
   会被丢弃，所以结构史最新点天然停在上一完整周）。backfill 只在补采集空洞时再跑。
   **注意 token 的历史与日度序列不是一条线**：口径不同（实测同日
