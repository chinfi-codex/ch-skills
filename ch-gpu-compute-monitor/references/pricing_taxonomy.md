...
| ④ 标准报价 | `on_demand` / `spot` / `preemptible` / `committed_3m` | 云厂商公开挂牌价 | Lambda、Runpod、CoreWeave、Nebius | 运营商自己怎么定价、有没有促销压力 |
...
- `available_region_count`：Lambda 有容量的 region 数，看的是覆盖广度不是深度。
...
6. **单卡口径才能比。** Vast 的 `dph_total` 是整条 offer 的价（`num_gpus` 张卡），Lambda 的 `price_cents_per_hour` 是整机价。都必须除以 GPU 数，并把 `node_gpu_count` 一起存，让换算可审计。
7. **同费用范围才能比。** Vast 的 `dph_total` 裹着磁盘费与 SLA 溢价，`search.gpuCostPerHour` 才是纯 GPU 费。默认取纯 GPU 费（`price_scope=gpu_only`）；Lambda 的整机价含 NVMe，只能标 `bundled`，与 Vast 的纯 GPU 价对比时要说明这层差异。
