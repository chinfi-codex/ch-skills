...
`CollectorError`，绝不返回 0 或沿用前值。逐 provider 价差带与应用榜都是补强不是核心，
取不到时降级成一条 note，不拖垮整次采集。**降级的边界要划在整段外面**：应用榜的解析
与逐行构造必须被同一个 try 包住，逐行校验散在 try 外面的话，任一应用缺
`total_requests` 就会把整个源判失败，连带 500 多行核心模型观测一起不落库。
另外只有全部行构造成功才写进结果——半截行落库会在库里留下一份残缺的当日榜单，
比没有更难发现。
