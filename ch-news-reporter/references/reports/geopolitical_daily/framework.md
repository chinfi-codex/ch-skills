...
frame:
  ...
  chokepoint_status: {type: map}                                    # {Hormuz, Bab-el-Mandeb, Black-Sea, Suez} 各自 open / degraded / selective / blockaded
                                                                    #   selective=军管选择性通行（一方武力管控下的单边航道/护航队仍走大流量），既非常态也非断流
  escalation_rung:   {}                                             # 军事升级阶梯位置 1-10（只读军事维度），10=大国直接开战/核使用
  econ_war_rung:     {}                                             # 经济战阶梯 1-10（制裁/断贸/通行费/资产冻结/保险与再保险），与军事阶梯并列、各读各的
...
output_sections: [战况速览, 主战线, 并行战线, 能源航运与市场重定价, 升级阶梯与核阈值]
```
...
| **W3 多战线扩大战** | **两个变量都要满足**：① 并行战线（俄乌、印太台海等）进入高烈度；② 它与主战线形成**同步升级**（共同触发、互为条件、或联盟成建制跨线卷入）。多通道同时中断是佐证、不是充分条件 | 并行战线低烈度；**或**虽高烈度但与主战线各打各的、无因果耦合（按下方「独立高烈度」处理，不进 W3） |
...
- **通道状态（chokepoint_status）**：霍尔木兹 / 曼德海峡 / 黑海 / 苏伊士 各自 open / degraded / **selective** / blockaded，是能源航运传导的结构化底座。
  - **selective（军管选择性通行）**：通道被一方武力管控，多数船只走不了，但管控方开的单边航道 / 护航队仍走大流量。跟 blockaded 的差别是**物理流量还在**，跟 degraded 的差别是**通行权由交战方发放**——写 blockaded 会高估断供、写 degraded 会低估强制性，所以单列一档。判据：出现受控通道且实际流量达常态量级的一半上下。（2026-08-20 实证：霍尔木兹标 blockaded，同期美军单边航道日均约 1000 万桶。）
- **「独立高烈度」是 W1/W2 与 W3 之间的过渡态，不是 W3**：并行战线自己烧得很旺、通道也断了，但与主战线没有因果耦合——这时 path 留在主战线对应的档（W1/W2），把 W3 概率抬上去，并在 `signal_watchlist` 写明「什么事件会让两条线耦合」。**别因为多个通道同时封锁就跳 W3**：同时封锁只证明烈度，不证明同步。（2026-08-20 实证：霍尔木兹 + 黑海双双 blockaded、俄乌独立高烈度但无同步升级 → path 维持 W1、W3=27%。）

### 台账锚点与母题（防膨胀，务必守）

open 跟踪项软上限 `open_budget=12`（见 `config/report_profiles.yaml`）。战时事件密度高，台账最容易在这里失控，两条纪律：

- **每条 `tracking_item` 挂一个 `theater_id`**（战线锚点，取自 `war_theaters` 的 name，如 `theater_id: Hormuz-energy-war`；纯通道类可挂 `chokepoint: Black-Sea`，既不属战线也不属通道的宏观/供应链项挂 `theater_id: non-theater` 并优先考虑该不该留在地缘台账里）。**同一 `theater_id` 下 open ≥2，就必须收进一个母题**——父项占 1 个预算位、子项各留自己的到期时钟与 status（机制见 `../watchboard.md §4`）。
- **不挂锚点就看不见成簇，这是实证结论**：AI 日报加了同构的 `vector_id` 后，open 从 19 收敛到 9-10（预算 10）并稳定守住；地缘在只有母题、没有锚点的状态下，2026-08-20 仍是 23 条顶层 open（预算 12 的近 2 倍），其中 OPEC+ 增产、亚太去中东化、伊朗战争经济、阿联酋断贸四条同属经济战簇却各开一条，而 `T-073 莱茵河水位`自己都标注「非地缘断点」还占着顶层位。

**别靠加预算解决膨胀**：先按 `theater_id` 归并，再把连续 3 期无实质进展的项转 `expired`。超预算且母题零使用时 `save_report_state` 会打 ⚠️ 醒目告警；上一期的子项被摊回顶层会被**硬拦截**（要合法晋升就写 `promoted_from` + `promote_reason`）。

### 升级阶梯与核阈值（v2 新增核心块）

战时最需要一条清晰的**升级阶梯**，把"再往上一格是什么"写死，避免每天在散文里临时判断：

**军事阶梯（`escalation_rung`）**：1-2 封锁威胁/口头 · 3-4 封锁执行（黑航/收费）· 5-6 实弹化封锁 + 代理人袭击 · 7 强行通航/成规模互击本土 · 8 联盟成建制卷入/多通道中断 · 9 核设施被实际打击 · 10 核使用/大国全面开战。

**经济战阶梯（`econ_war_rung`，与军事阶梯并列、各读各的）**：1-2 制裁威胁/口头 · 3-4 定向制裁（实体清单/个人）· 5-6 部门级制裁 + 通行费/关税化 · 7 金融管道封锁（SWIFT/银行/影子船队）· 8 区域集体断贸（GCC 级）· 9 全面封锁式制裁 + 保险与再保险停摆 · 10 资产没收/经济体被整体切离。

**为什么拆两条**：战时经常一条腿停、另一条腿继续升。军事阶梯纹丝不动而制裁一路加码时，只给一个 rung 读数会造成「局势没变」的错觉，模型就会在散文里自造「经济战腿」「经济战双引擎」这类框架外词汇——**那是框架缺格的信号，不是文采**（v1 的 sub_branch 溢出就是这么开始的）。（2026-08-07~20 实证：`escalation_rung` 连续 14 天钉死 6.0，同期经历 GCC 断贸 + 史上最严厉制裁，经济战维度完全没被记录。）
...
- **台账不进正文**：上一期 open 母题与子项照常逐条结算、换代日照常按 `migration_plan` 留痕（`framework_switch` 字段），但这些都写进 watchboard，报告不设「本期变更」段。结算若改变了 path、战线或通道判断，写进对应正文板块。
