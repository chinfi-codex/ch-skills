...
- **A. 启动声明**：先跑 `python scripts/thesis_scan.py --evidence reports/evidence_<code>.json`，然后向用户输出 `[Deep 模式启动]` 协议（命题候选 + 高优先旗标数 + 调研预算 + 计划 probe 前 3–5 条），见 `references/deep_mode.md §0`。
...
- **C. 报告内必填产物（3 项）**：
  ...
  2. 正文内嵌 **≥ 3 条** `==深度调研发现==` 块（每条标注【原文】出处 + 章节/页 + 一句判断）。
  3. 报告末尾必填 **«附：深度调研索引»** 表格——列出每条发现的（编号 / 命题或旗标 / 原文出处 / 引文摘要 / 模型判断 / 置信度）。
- **D. 完成声明**：报告之外向用户输出 `[Deep 模式完成]` 收口（读 N 篇 / 联网 K 次 / 嵌入 M 条 / 索引 ✅）。

A/C/D 任一缺失即视为骨架版冒充 Deep——必须补齐再交付，不得提前结案。完整 playbook、命题原型表、`--section` 用法、subagent 模板、停止条件见 `references/deep_mode.md`。
...
4. **三情景推演**（§5.4）——三情景的推算变量与表头**必须沿用主锚**（PE / PS / PB / EV-EBITDA / EV-ARR / 股息率 / rNPV / SOTP 七种模板,见 §5.4.A 表），辅助锚只能在表后做对照,不得反过来作主路径
...
- **主线归属修正后总结论**：[当前价格是否合理]
- **总体置信度**：[高/中/低]，依据 [六项的完备度 + 数据时效 + 第一部分财务异常项]
...
> 此节是 Deep 模式无法被静默省略的"足迹"。每条"深度调研发现"都必须在此登记一行。

| # | 命题 / 旗标 | 原文出处（标题 + 章节 / 页） | 引文摘要（≤ 30 字） | 模型判断 | 置信度 |
|---|---|---|---|---|---|
| 1 | [A1/A2/B/异常类型] | [年报 / 季报 / 公告标题 + 章节] | "..." | [一句证实/证伪 + 数字] | 高/中/低 |
| 2 | ... | ... | ... | ... | ... |
| 3 | ... | ... | ... | ... | ... |
...
写完报告 *交付前* 必须勾完以下 4 项；任一漏项必须回头补齐，不得提前结案：

- [ ] 是否在报告之外向用户输出了 `[Deep 模式启动]` 协议？（命题候选 + 高优先旗标数 + 调研预算 + 计划 probe）
- [ ] 报告顶部「核心判断」第一行是否有 `> 本报告为 Deep 模式产出：…` 标识？
- [ ] 正文是否嵌入 **≥ 3 条** `==深度调研发现==` 块，每条带【原文】出处 + 章节 / 页 + 一句模型判断？
- [ ] 报告末尾是否有完整的 **«附：深度调研索引»** 表格（每条 == 块对应一行）？
- [ ] 是否在报告之外向用户输出了 `[Deep 模式完成]` 收口？（读 N 篇 / 联网 K 次 / 嵌入 M 条 / 索引 ✅）

读了 PDF、跑了 thesis_scan 但报告里只多了几句"模糊判断"——这是骨架版冒充 Deep，不算合格交付。
