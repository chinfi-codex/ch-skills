...
| 机构调研 | `institutional-research` | **默认走巨潮 `tabtype=relation` 频道**取投资者关系活动记录表（含 PDF 全文，加 `--with-text`）；`--research-source tushare` 回落到 `stk_surv`（仅结构化摘要、需 TUSHARE_TOKEN）。用于观察调研频率、参与机构、接待层级和纪要内容 |
...
- `tabtype=fulltext` 用于常规公告全文列表。**注意两件不同的事**：①「深交所互动易问答」正文用 `interactive-qa` 数据集直接抓取（见 §7.3），不走 relation；②「投资者关系活动记录表（机构调研纪要）」**只在 `tabtype=relation` 频道，fulltext 检索返回 0 条**，必须用 `institutional-research` 数据集（默认已走 relation）或显式 `fetch announcements --tabtype relation`。**不要因为 fulltext 扫不到就认为公司没有调研。**
...
2. **扫增量信源 + 分析**：拉自上次更新日以来的新证据——新公告 / 定期报告、**机构调研（`fetch institutional-research <code> --start-date <上次更新日>`，默认走 relation 频道；命中后按需加 `--with-text` 取纪要正文）**，**并默认加拉互动易 QA**（`fetch interactive-qa <code> --date-from <上次更新日>`，看公司对跟踪看点与市场新关切的最新公开表态）。互动 QA 是追踪更新的**默认信源范围**，用来和公告/研报叙事**交叉印证**（防单一来源偏差），但它是单方口径、非承诺，冲突时以公告/定期报告为准（见 §7.3）。据此照常走定性/定量框架。
...
使用 `institutional-research` 观察市场关注度和产业问题焦点。**该数据集默认从巨潮 `relation` 频道取投资者关系活动记录表；常规 `fulltext` 公告检索不返回这类文件**——追踪时必须单独走这条线，否则会漏掉全部调研信息。

- **先看有没有，再看说了什么**：定期报告里「报告期内接待调研、沟通、采访等活动登记表」若勾选「不适用」，说明该期为零；**这是一个会变化的维度，下一轮必须主动复查是否已经不为零**。
- **纪要正文才是信息所在**：加 `--with-text` 抽取 PDF 全文。参与机构名单、接待层级、Q&A 原文（订单口径、交付进度、定价变化等）都只在正文里，结构化字段给不了。
...
- **口径纪律（强制）**：调研纪要是**公司单方口径、非公告承诺**，权重低于公告与定期报告。① 与公告/报表冲突时**一律以公告与报表为准**，并把冲突显式写进报告，不得静默采信纪要；② 纪要中"剔除某某后的经营利润""可用资金体量""违约金/兜底条款"等自造口径或未入公告的条款，只能标为**待验证**，不得据此调整盈利预测或情景概率；③ 同一批问答分多日发布时，以**最后一版**措辞为准（合规收敛通常发生在后续版本）。
