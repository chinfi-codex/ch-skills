...
**输出落位**：报告第一部分新增「成长成功率」子节（A/B/C + 档位）；并按 `growth_success_rate.md §4` 接回定量——调三情景概率权重（§5.4）、定 B 型期权计入比例、上/下调估值结论置信度（§5.6）。每条发现按 §6 模板做成结构化 `==深度调研发现｜工单号｜主题｜置信度：高/中/低` 卡片，并登记进「深度调研索引」。
...
- **subagent 必须按下面固定模板回报**（缺字段就退回重发，避免主 agent 拿到松散文字最后整段丢掉）：
  ...
  ```
  [调研项] <工单号 + 命题/旗标 ID 或一句话描述>
  [原文出处] [年报/季报/公告标题](adjunct_url 或 download_url) + 章节(如「管理层讨论与分析 · 经营情况」) + 页码/段落
  [原文引文] "<≤2 句直接引用，加引号>"
  [模型判断] <一句话：证实/证伪/部分支持，含数字>
  [影响] <一句话：如何影响成长成功率、三情景概率、估值置信度或风险判断>
  [置信度] 高 / 中 / 低（缺哪类信息说明）
  ```

  主 agent 要把这个块压成下面的正文卡片，且**每个 ✅ 工单项最多一张卡**：
  ...
  来源链接必须优先用 `report-text` / `announcement-text` 返回的 `download_url` 或 `adjunct_url`；联网资料用原网页 URL。确实没有 URL 时，写 `来源：《标题》（原文链接缺失） · 章节/页`，并在「深度调研索引」保留缺口。主 agent 收到 K 个 ✅ 结论块，就有 K 条深度发现，不会"读了但没写"。
...
**Deep 模式合格交付的报告必须同时具备以下 3 项产物**（缺一即视为骨架版冒充 Deep，必须补齐）：

1. **正文嵌入结构化 `==深度调研发现｜...` 卡片、覆盖工单每条 ✅ 完成项**（就近落到成长诊断 / 核心看点 / 成长成功率 / 主线归属 / 风险；每张卡只写 `来源 / 原文 / 判断 / 影响` 四行；`来源` 标题必须带原文链接）。绑定标准是"查了几条体现几条"，不是凑满 3 条——实质发现至少 3 条，否则算调研不足。
2. **报告末尾必填 «附：深度调研索引» = 完整性账本**——工单每条**执行过**的项登记一行（工单号 / 命题·旗标·维度 / 状态 ✅⚠️✗ / 原文出处标题链接 / 引文 / 判断 / 置信度）；**索引行数 == 已执行工单项数**，少一行即漏，⚠️/✗ 注明状态并汇入置信度。这一节是 Deep 无法被静默省略的"足迹"。
3. **报告之外**额外向用户输出 `[Deep 模式启动]` 与 `[Deep 模式完成]` 两段协议（§0）。

> 反例（常见失败模式）：跑了 thesis_scan、读了 PDF，但写报告时只在骨架里加了几句模糊判断、没标【原文】、没加索引——这等同于 Deep 模式没跑。下次必须按上面 3 项清单收口。
