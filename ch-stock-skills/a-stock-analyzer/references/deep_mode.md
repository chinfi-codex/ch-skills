...
· 联网检索：K 次
· 嵌入报告的"深度调研发现"：M 条（落点章节：成长诊断 §4 / 核心看点 §5 / 主线归属 §6 …）
· 附「深度调研索引」：✅
```
...
2. **正文内嵌至少 3 条 `==深度调研发现==` 块**（高优先命题/旗标全覆盖；每条带【原文】出处 + 章节/页 + 一句模型判断；落点对应章节自由——成长诊断、核心看点、主线归属、风险等）。
3. **报告末尾必填章节 «附：深度调研索引»**——表格列出每条深度发现的「编号 / 命题或旗标 / 原文出处 / 引文摘要 / 模型判断 / 置信度」。这一节是 Deep 模式无法被静默省略的"足迹"。
