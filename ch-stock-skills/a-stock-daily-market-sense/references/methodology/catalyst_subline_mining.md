...
- 只覆盖当日已由量价锁定的 ★★★ 主线。★ 与 ★★ 方向不搜索、不推演，也不进入 2.2。
...
产业逻辑强但盘面未确认时，只列为研究线索；盘面有异动但产业逻辑薄弱时，写成事件脉冲或待核映射。两种情况都不能改变 2.1 主线星级。

## 5. 输出纪律

`2.2` 不使用横向大表格。仅对每条三星主线按下列纵向结构输出：
...
- 只输出 ★★★ 主线；若当日没有三星主线，2.2 整节不输出，包括标题与兜底句。不得用 ★★ 主线补位。
