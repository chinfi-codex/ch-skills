...
- 折扣启动组重点引用前高折扣（`discount_after_high`，连同 `prev_high_date` 前高日、`prev_high_close` 前高收盘、`post_low` / `post_low_date` 回撤最低价及其日期、`low_to_target_days` 最低点距大涨日交易日数）、现价相对前高 `close_vs_prev_high`、缩量比（`pre_volume_contraction_ratio`）、当日相对前 5 日的放量倍数（`amount_vs_prev5_ratio`）、成交额与总市值；可结合 `pre_ret_5d` 判断今日之前是充分调整还是仍在加速——脚本只做硬筛，“回撤是否到位、最低点是否够新鲜、现在是否仍折价、放量是否可信”由模型判断，必要时点明待验证点。
