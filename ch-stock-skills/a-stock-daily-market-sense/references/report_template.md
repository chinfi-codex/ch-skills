...
| 股票 | 涨幅 | 成交额(亿) | 总市值(亿) | 前高收盘 | 回撤最低(日期) | 距大涨日 | 前高折扣 | 现价/前高 | 缩量比 | 放量倍数 |
|---|---:|---:|---:|---:|---|---:|---:|---:|---:|---:|
|  |  |  |  |  |  |  |  |  |  |  |

前高折扣 = `discount_after_high`（前高之后最低价 ÷ 前高收盘价，前高 = 最近 200 日收盘最高那天，落在 0.6~0.85）；回撤最低日期 = `post_low_date`、距大涨日 = `low_to_target_days`（须 ≤5 个交易日）；现价/前高 = `close_vs_prev_high`，反映现价是否仍折价；缩量比为 `pre_volume_contraction_ratio`，放量倍数取 `amount_vs_prev5_ratio`（相对前 5 日缩量期）。无候选时写“暂无命中”。
