...
泳道图的数据来自台账、不来自报告，所以漏掉 `record` 不会报错，只会画出一条最后一列全空的带子——页头还照写「截至本期 <当日>」。2026-08-19 就这么发出去了，而那天恰恰是科技链集体退潮、最该留下状态转移的一天，三段渲染门禁全绿也照样没看出来。

现在 `render_report_html.py` 在注入区块前调用 `lifecycle_currency_gap(payload, asof)`：台账有窗口数据、却查不到报告当日的记录时直接 `ContractError`，不产出 HTML。判定口径：

- 当日任一主线有 `theme_daily_state` 记录 → 通过；
- 全面退潮日没有主线记录，但 `theme_market_day` 有当天那行 → 通过；
- 台账整体为空、连不上库、文件名里没有日期 → 照旧静默跳过整个区块（可用性是可选的，当期性不是）；
- 历史回补等确实不需要台账的场景 → 显式加 `--no-lifecycle`。
