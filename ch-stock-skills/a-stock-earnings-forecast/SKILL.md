...
6. **生成报告期 HTML**：`python3 scripts/render_period_html.py --period 20260630` → `reports/forecast_<period>.html`。页面基础视觉统一加载仓库 `shared/html_report/themes/default.css`（同步安装后位于 `scripts/_shared/html_report/themes/default.css`），skill 内只保留 Earnings Forecast 的主从布局与交互增量样式；字号层级需保证标题、筛选器、列表、详情和脚注可读，宽屏容器充分展开。**一期一页、每次从累积缓存整页重渲染**：读 evidence + enrich + `forecast_verdict` + `forecast_theme_trend` + `forecast_period_overview` + `forecast_daily_cache`，实时 join `theme_daily_state`。页面为「标题 → **产业结构综述**（模型全文 + 判于日期 + 样本变化待复判徽标）→ 过滤条 → **左列表+右详情**双栏」（无 KPI 卡片）：左列表**默认按发布时间排序**（按最新公告日倒序、日期组头展示；用户可切换按主线分组或平铺），断层股整行强标注（**非 K 线信息展示统一红=上涨/强、绿=下跌/弱：向上=业绩超预期跳空↑强表现红，向下=不及预期跳空↓弱表现绿**）+ 未回补 D+n / 主线内徽标；过滤器支持**跳空未回补**（只显示向上跳空且未回补的超预期个股）。右侧 sticky 详情头部下方是**年化PE醒目条**（大号数字=预告中值年化PE，副行给总市值、年化净利换算与滚动PE对照；年化净利≤0 显示"仍亏损，PE 无意义"），其后为 **SVG 日K线**（约130根+量，前复权 qfq，红涨绿跌，蓝虚线=公告日标注，落在公告日前一交易日——如公告日 7.3 则标 7.2，橙虚线=公告前收盘）、精简的股价断层（只留跳空+公告后累计，其余看K线）、**所属主线报告期行业趋势块**（机械层：强/弱表现分布+净方向+成员样本；判断层：模型判的方向+强弱侧归因+交叉验证结论）、业绩/主线/判分/变动原因全档。渲染只投影+机械计数，断层方向/两型/行业趋势净值都是阈值切分；行业趋势的定性来自 `forecast_theme_trend` 台账里模型的判断，渲染只展示不代判。
...
| `--start-ann` / `--end-ann` | 公告日扫描窗口（逐交易日调 `forecast(ann_date=)`） | period−45d ~ min(今天+1天, period+75d) |
...
- `stocks[]`：每股一条，按当年累计同比降序。
  ...
  - `valuation`：预告中值年化 PE 估值参照——`total_mv_yi`+`mv_asof`（最新交易日总市值）、`annualized_np_yi`+`annualize_label`（年化净利=中值÷报告期季数×4）、`pe_annualized`（+`pe_annualized_note`：`ok/np_missing/np_nonpositive/mv_missing`，年化净利≤0 不给 PE）、`rolling_np_yi`/`pe_rolling`（滚动口径=上年年报实际+本期中值−上年同期实际，季节性对照）。读法与坑见 `references/methodology.md` §十一。
...
- **增量逻辑**：`forecast_scan` 只抓 `forecast_fetch_log` 里没有的公告日，外加最近 `--refetch-days`（默认 3）天始终重扫（默认扫描到运行日+1，抓当日及盘后按次日公告日期入库的新披露与修订版）；然后从 `forecast_cache` 读回该报告期**全量累积**结果做拆解。个股实际季报按 `(ts_code, period)` 命中缓存，只有新出现的个股才发 income 请求。`cninfo_enrich` 按 `(code, period)` 复用已下载解析的公告，不重复下 PDF。
...
- **引用 PE 必标口径**：写“年化PE 22 倍”要注明是预告中值年化（中值÷报告期季数×4，未调季节性），与静态PE/PE-TTM 不同；扭亏小基数的数百倍 PE 按低基数失真处理、改讲绝对额，扣非 << 归母的票要提醒年化PE被一次性损益压低（§十一）。
