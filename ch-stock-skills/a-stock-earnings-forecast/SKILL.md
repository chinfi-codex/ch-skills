...
6. **生成报告期 HTML**：`python3 scripts/render_period_html.py --period 20260630` → `reports/forecast_<period>.html`。**一期一页、每次从累积缓存整页重渲染**：读 evidence + enrich + `forecast_verdict` + `forecast_theme_trend` + `forecast_period_overview` + `forecast_daily_cache`，实时 join `theme_daily_state`。页面为「标题 → **产业结构综述**（模型全文 + 判于日期 + 样本变化待复判徽标）→ 过滤条 → **左列表+右详情**双栏」（无 KPI 卡片）：左列表**默认按主线分组**（组头显示**报告期行业趋势**：机械计数 `↑偏强/↓偏弱/⇅分歧` + 强N/弱M，已判主线再加模型趋势 `判·向上/向下/分化/证据不足`（hover 见交叉验证结论，成员变化标†待复判），主线按净方向降序），断层股整行淡色强标注（**向上=业绩超预期跳空↑强表现绿、向下=不及预期跳空↓弱表现红**）+ 未回补 D+n / 主线内徽标；也可切按断层分组或平铺。右侧 sticky 详情含 **SVG 日K线**（约130根+量，红涨绿跌，蓝虚线=公告反应日、橙虚线=公告前收盘）、精简的股价断层（只留跳空+公告后累计，其余看K线）、**所属主线报告期行业趋势块**（机械层：强/弱表现分布+净方向+成员样本；判断层：模型判的方向+强弱侧归因+交叉验证结论）、业绩/主线/判分/变动原因全档。渲染只投影+机械计数，断层方向/两型/行业趋势净值都是阈值切分；行业趋势的定性来自 `forecast_theme_trend` 台账里模型的判断，渲染只展示不代判。
...
- **落库的东西**：`forecast_cache`（每股最新一版预告）、`forecast_fetch_log`（已扫过的「报告期×公告日」水位）、`forecast_income_cache`（实际季报归母净利/营收，含 NULL 行标记「查过但无数据」，避免次新股每次重查）、`forecast_daily_cache`（预告股日线，供断层计算；首次抓公告前 400 自然日历史，此后每天只补尾部新 K 线）、`forecast_basic_cache`（名称/行业）、`forecast_enrich_cache`（cninfo 扣非/原文）、`forecast_verdict`（模型判分 tier + 主线归属 + 证据指纹，增量判、待复判可查）、`forecast_theme_trend`（模型判的报告期行业趋势：方向 + 强/弱侧归因 + 交叉验证 + 成员数快照）、`forecast_period_overview`（模型写的产业结构综述全文 + 样本指纹，一期一条）。`theme_registry`/`theme_daily_state` 是 daily-market-sense 维护的主线台账，本 skill **只读**。
