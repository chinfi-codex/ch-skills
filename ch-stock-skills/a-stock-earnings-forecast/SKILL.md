...
6. **生成报告期 HTML**：`python3 scripts/render_period_html.py --period 20260630` → `reports/forecast_<period>.html`。**一期一页、每次从累积缓存整页重渲染**：读 evidence + enrich + `forecast_verdict` + `forecast_daily_cache`，实时 join `theme_daily_state`。页面为「标题 → 过滤条 → **左列表+右详情**双栏」（无 KPI 卡片）：左列表**默认按主线分组**（组头显示**报告期行业趋势**：按主线聚合成员断层方向、`↑偏强/↓偏弱/⇅分歧` + 强N/弱M，主线按净方向降序），断层股整行淡色强标注（**向上=业绩超预期跳空↑强表现绿、向下=不及预期跳空↓弱表现红**）+ 未回补 D+n / 主线内徽标；也可切按断层分组或平铺。右侧 sticky 详情含 **SVG 日K线**（约130根+量，红涨绿跌，蓝虚线=公告反应日、橙虚线=公告前收盘）、精简的股价断层（只留跳空+公告后累计，其余看K线）、**所属主线报告期行业趋势块**（该股所在主线的强/弱表现分布+净方向+成员样本）、业绩/主线/判分/变动原因全档。渲染只投影+机械计数，断层方向/两型/行业趋势净值都是阈值切分，机会与行业趋势定性由判分台账与报告承担。
