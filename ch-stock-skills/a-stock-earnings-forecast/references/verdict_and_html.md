...
一期一个 `forecast_<period>.html`，每次从累积缓存**整页重渲染**（幂等，不 patch DOM）。结构为「KPI → 过滤条 → **左列表 + 右详情**双栏」，按「业绩×股价断层×主线」论点组织：

- **KPI**：已披露、断层（加速/低位两型计数）、未回补、未反应、已归属主线、分档与待判/复判。
- **左侧列表**（每股两行：名称/代码/分档/回补徽标/主线内徽标 + 类型·三口径·跳空·主线摘要），三种分组切换：
  - **按断层分组**（默认，来自 `price_reaction` 的机械 facet）：趋势加速型断层（公告前一年分位 ≥ `--pos-split` 默认50，已有表现+业绩推动续升）/ 低位启动型断层（分位 < split，低位被业绩点火）/ 断层·位置数据不足（次新 history_short）/ 业绩强但股价未反应（已判强/中、无跳空、公告后累计 < `--muted-max` 默认3% ——潜在未定价）/ 其余。组内"主线内"优先、按跳空幅度降序（断层+主线内排最前=高胜率锚）。
  - **按主线分组**：每条在场主线一段（名称+当前★状态），末尾无归属/未判。
  - **平铺**。过滤器：搜索 / 有断层 / 未回补 / 未反应 / 主线内 / 分档。
- **右侧详情**（sticky，点击列表切换）：
  - **K线图**（SVG 自绘，~130 根日K + 成交量，红涨绿跌）：**蓝虚线=公告反应日、橙虚线=公告前收盘**——断层缺口一眼可见；数据来自 `forecast_daily_cache` 内嵌。
  - 股价反应块（跳空/当日/量比/公告后/断层状态 D+n/位置分位/20日动量/公告前收盘）、业绩块（归母中值/三口径/扣非/营收 trailing）、归属主线（状态+理由）、判分（reason/caveat）、业绩变动原因全文。
- 自包含单文件、无外链 CDN、支持深色模式。两型分类是**机械阈值切分**（分位 50 二分、跳空≥2%），渲染只投影 evidence+verdict+主线状态，机会与否由判分台账与报告承担，不给买卖建议。断层新鲜度（D+n）必须可见——刚披露的"未回补"没经过时间检验。
