...
6. **全市场都要披露，所以样本没有偏差，但也不能全读。** 5500 家全在样本里——这是相对预告的最大优势（产业结构综述可以谈全经济体量，不用写"触发式披露样本偏向剧烈变化"的 caveat）。代价是模型读不完，所以脚本出**机械漏斗**：决策包 `stocks[]` 只放候选（默认 rank_score 前 200），全样本进 `qreport_universe_<period>.json` 供查询与渲染。
...
5. **判分 + 质量成色 + 兑现度 + 主线匹配 + 行业趋势 + 产业综述落台账**：跑 `python3 scripts/verdict.py context --period 20260630` 拿到待判集合（新披露 + 追溯调整/增速漂移的待复判）、主线注册表（daily-market-sense 的 `theme_registry`，只读）、待判主线的强/弱/无断层成员简报、产业综述状态与全样本 `industry_summary`。模型逐只判 **`tier`（强/中/观察/剔除）+ `quality_call`（扎实/尚可/存疑/虚高）+ `fulfillment`（超预告上限/落区间上沿/符合/落区间下沿/低于预告/无预告）+ `theme_id`**，逐主线判 `theme_trends[]`，全样本写 `period_overview`。写 `reports/qreport_verdict_<period>.json`，再 `python3 scripts/verdict.py record --period 20260630 --input reports/qreport_verdict_<period>.json` 落 `qreport_verdict` / `qreport_theme_trend` / `qreport_period_overview`。台账增量累积，每天只判 context 列出的增量。细则见 `references/verdict_and_html.md`。
6. **生成报告期 HTML**：`python3 scripts/render_period_html.py --period 20260630`；发布链路必须加 `--require-ann-cutoff "$NEXT_ANN_DATE"`。一期一页、每次从累积缓存整页重渲染：产业结构综述 → 过滤条 → 左列表 + 右详情（详情以利润质量三件套开头，然后兑现度、资产负债表前瞻、K 线与断层）。K 线走同级 `qreport_<period>.klines/` 分片，**发布时必须连同该目录一起复制**。
...
- `meta`：报告期、`quarters_elapsed`、`released_count` / `with_statements` / `disclosure_progress_pct`（披露进度，写报告必须交代）、`ann_cutoff` + `ann_cutoff_stock_count` + `clock_timezone=Asia/Shanghai`、`shortlist_rule`、`thresholds`、`fetch_stats`、`data_notes`。`primary_source=tushare_statements`、`pdf_role=on_demand_only` 固定记录源角色。
- `industry_summary[]`：按 stock_basic 行业的**全样本**聚合（家数、增长/下滑/亏损家数、加速、毛利上下行、现金覆盖、订单、断层上下家数、三口径增速中位数、按利润规模取前 5 的成员样本）——产业结构综述的原料。
- `stocks[]`：候选池，每股一条。
  - `source`：`authority=tushare_statements` + `sources`（哪几个接口已落地）+ CNInfo 标题/链接/是否更正后。
...
- **产业结构综述可以谈全经济体量**：季报是强制全市场披露，不需要预告那套"触发式披露样本偏差"caveat；但**必须写清截至当前的披露进度**（`disclosure_progress_pct`），进度 30% 时的结构判断和 99% 时不是一回事。
