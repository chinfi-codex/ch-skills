# 判分台账 + 质量成色 + 兑现度 + 主线匹配 + 行业趋势 + 产业综述 + 报告期 HTML

把模型的判断结构化落库（`qreport_verdict` / `qreport_theme_trend` / `qreport_period_overview`），再投影成一期一页的 HTML。分档锚点见 `methodology.md` §二，行业趋势归因见 §九，产业综述见 §十；这里讲**台账格式、判分工作流、增量与陈旧语义、HTML 契约**。
...
## 四、产业结构综述台账（qreport_period_overview）

一期一条，全样本判断，成文文本直接落库。原料是 evidence 的 `industry_summary`（context 已透传）。方法论见 `methodology.md` §十。

- `record` 校验它是**非空字符串**（要成文，不是要点碎片），并快照样本指纹 `(总家数, 增长家数, 下滑家数)`。
- `context.period_overview.reason_to_judge`：`no_overview`（还没写）/ `stale_sample`（指纹变了）/ `null`（当前有效）。`prev` 附上一版全文，重写时在其基础上增量修正。
- **必须写披露进度**。季报没有预告那种触发式样本偏差，所以综述可以谈全经济体量结构；但进度 30% 和 99% 是两回事，不交代进度的结构判断是不负责任的。
- 综述不依赖主线台账，主线为空也照常写。
...
产业综述同理：任何新披露都会让指纹变成 `stale_sample`——这是设计使然，财报季综述本就该随披露进度演化。HTML 页首打「样本已更新，待复判」徽标。
...
页面结构：**标题与披露进度条 → 产业结构综述（模型全文 + 判于日期 + 当时样本数 + 待复判徽标）→ 过滤条 → 左列表 + 右详情双栏**。
...
- **左列表**（每股两行）：名称/代码/分档/质量成色徽标 + 断层标注 + 兑现度标注；第二行是「营收单季 · 归母单季 · 扣非 · 现金覆盖 · 披露日」。断层行整行淡色着色，**非 K 线信息统一红=上涨/强、绿=下跌/弱**。按 120 家一批渐进渲染，避免财报季全量样本一次性建超大 DOM。
  - 排序/分组：按机械得分排序（默认）/ 按披露时间 / 按主线分组（组头显示行业趋势）/ 按行业分组 / 平铺。
