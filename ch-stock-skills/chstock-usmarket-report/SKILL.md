...
- 收益口径：1 日收益比较当日与前一完整交易日收盘；5 日 / 20 日收益分别比较当日与 5 / 20 个完整交易日前的收盘（即 5 / 20 个收益区间）。盘中尚未完成的当日日线不进入计算。
- 量比口径：当日常规时段成交量 ÷ 前 20 个完整交易日的平均成交量；前序样本不足 20 日时返回空值，不用短窗口冒充 20 日量比。
- vs-QQQ 只在个股与 QQQ 的收益起止交易日完全一致时计算；日期错位时返回空值，并通过 `vs_qqq_1d_date_aligned` / `vs_qqq_5d_date_aligned` 标明。
...
- **取数被拒 ≠ 当天没异动**。Yahoo edge 偶发把请求判成机器人（403/429），脚本已自动重试并在 query1/query2 间切换；仍失败时 `market_wide_movers.degraded=true`，`degraded_note` 写明哪一侧挂了。此时涨方或跌方的池子是**空的但不代表真空**，必须在板块赚钱效应段写明"该侧扫描失败、样本缺失"，**不能**据此写成"今晚无上涨主线"。隔一会儿重跑一次通常就好了。
