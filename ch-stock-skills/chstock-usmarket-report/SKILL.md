...
所有 `is_abnormal=true`（\|当日\| ≥ 7%）的票**汇总进一张「异动扫描」表**（结构见输出规范），**不拆"观察池内 / 池外"**、按成交额降序、`归属`列标所属分组或"池外"；同时每只在其所属主题 / 分组处标 `★`。每只按下面的 Tavily 流程核查后，把"催化（来源·日期）"填进表的对应列。
...
**核查优先级（避免一晚几十只全查爆 token）**，按下面取并集后通常 10–18 只：① 观察池里的异动票必查；② 赚钱效应 / 新方向里 ★★ 及以上主题的领导股必查；③ dollar-volume 最大的 5 只必查；④ \|涨跌\| 最大的 5 只必查；⑤ 其余仍进异动扫描表、`催化`列标"未做联网核查"（不另起清单段）。表格脚注一句说明已查 n 只 / 未查 m 只即可。
...
- **不写方法论 / 口径 / 操作说明进正文。** "数据基础：……""`信号` 列读法：……""核查优先级：①……""空组省略不写占位行""scan_date / market_states……"这类是给你的执行指令、不是给读者的内容；除结尾那段固定数据来源脚注外，正文不要出现这类注释段或引用块（`>` blockquote）。该交代的口径直接融进判断的人话里，或干脆省掉——读者要的是结论不是流程。
...
|---|---:|---:|---:|---:|---:|:--:|
| ... | ... | ... | +X.XX% | ... | 0.XX | ↑↑＋ |
（每组一段点评 + 一张表列全部成员；空组整组省略。HTML 渲染会自动删掉这些分组表、只保留每组点评，逐股明细交给热力墙看，故点评要能独立成立、别只说"见表"。）
...
**一张表汇总当晚所有 ±7% 异动票，不拆"观察池内 / 池外"**；按成交额降序；`归属`列填所属观察池分组名或"池外"。核查优先级与 Tavily 流程见领域方法论 §6。
